...
/**
 * \brief Output label structure containing classification results
 */
typedef struct {
    char label_name[MAX_LABEL_NAME_LENGTH]; /**< Null-terminated class label
                                               string */
...
int model_config = VEHICLE_MODEL;
...
static const char* RPS_CLASS_NAMES[] = {"Paper", "Rock", "Scissors"};
 ...
 */
static void DrawClassLabelOnImage(uint8_t* imageBuffer, uint32_t img_width,
 ...
 */
void prepare_image_input_custom_squeezenet(const uint8_t* image) {
...
void runner_init(RunnerContext& ctx,
    ...
    /* Model initialization complete - input preparation will happen in
     * run_model() */
...
/**
 * \brief Execute model inference on input frame
 * \param[in,out] ctx        Runner context
 * \param[in]     model_pte  Pointer to model PTE data
 * \param[in]     frame_data Pointer to input frame data
 * \param[in]     frame_size Size of input frame in bytes
 * \return true if execution succeeded, false otherwise
 */
bool run_model(RunnerContext& ctx, const void* model_pte,
               const uint8_t* frame_data, /* Add frame data parameter */
               size_t frame_size) {
    Error status;
    int n = 0;
    for (n = 0; n < num_inferences; n++) {
#if ENABLE_TIME_PROFILING
        pre_process_time = profiler_start();
#endif
        prepare_image_input_custom_squeezenet(
            frame_data); /* Process the captured frame */

        std::vector<std::pair<char*, size_t>> input_buffers;
        /* prepare the input as like cmsis executorch example */

        input_buffers.emplace_back(reinterpret_cast<char*>(input_tensor_data),
                                   sizeof(input_tensor_data));

        /* Update input tensors with new frame data */
...
#if ENABLE_TIME_PROFILING
    post_process_time = profiler_start();
#endif
...
/**
 * \brief Main ExecuTorch runner entry point
 * \param[in]  frame_data    Pointer to input frame data (RGB888)
 * \param[in]  frame_size    Size of input frame in bytes
 * \param[out] out_buf       Pointer to output buffer for classification result
 * \param[in]  out_buf_size  Size of output buffer in bytes
 * \param[out] classify_flag Pointer to classification success flag
 * \return 0 on success, -1 on error
 */
int executorch_runner_main(uint8_t* frame_data, size_t frame_size,
                           uint8_t* out_buf, size_t out_buf_size,
                           bool* classify_flag) {
    /* Make all these static so they persist between calls */
    static RunnerContext ctx;
    static Box<BufferDataLoader> loader;
    static Box<Result<Program>> program;
    static bool initialized = false;
    vStreamStatus_t v_status;
    size_t pte_size = sizeof(model_pte);
    uint8_t* outFrame;
    uint8_t* img_buf = frame_data;

    memset(out_buf, 0, out_buf_size);

    /* Initialize only once */
    if (!initialized) {
        /* Initialize loader using Box::reset */
        loader.reset(model_pte, pte_size);

        /* Load program - construct Result in-place using Box::reset */
        program.reset(Program::load(&loader.value()));

        if (!program->ok()) {
            printf("Program loading failed @ 0x%p: 0x%" PRIx32 "\n", model_pte,
                   (uint32_t)program->error());
            return -1;
        }

        /* Now initialize the rest, passing the Program pointer */
        std::vector<std::pair<char*, size_t>> input_buffers;
        runner_init(ctx, input_buffers, pte_size, &program->get());
        initialized = true;
    }

    /* Pass frame data to run_model */
    bool model_ok = run_model(ctx, model_pte, frame_data, frame_size);

    memcpy(out_buf, &output_label, sizeof(output_label_t));
    *classify_flag = classify_object;

    ET_CHECK_MSG(model_ok == true, "Problem running model");
    snprintf(output_string, sizeof(output_string), "%s-%d",
    ...
    /* Copy image to display framebuffer */
    image_copy_to_framebuffer(
        frame_data, IMAGE_WIDTH, IMAGE_HEIGHT, outFrame, DISPLAY_WIDTH,
        DISPLAY_HEIGHT, (DISPLAY_WIDTH - IMAGE_WIDTH) / 2,
        (DISPLAY_HEIGHT - IMAGE_HEIGHT) / 2, IMAGE_FORMAT_RGB888,
        DISPLAY_FLIP_HORIZONTAL, DISPLAY_FLIP_VERTICAL, DISPLAY_SWAP_RB);
