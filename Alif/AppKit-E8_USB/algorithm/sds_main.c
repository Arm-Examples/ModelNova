...
/**
 * \brief Execute the ExecuTorch runner main function
 * \param[in]  frame_data      Pointer to input frame data
 * \param[in]  frame_size      Size of input frame data in bytes
 * \param[out] out_buf         Pointer to output buffer
 * \param[in]  out_buf_size    Size of output buffer in bytes
 * \param[out] classify_output Pointer to classification output flag
 * \return 0 on success, non-zero on error
 */
extern int executorch_runner_main(const uint8_t* frame_data, size_t frame_size,
                                  uint8_t* out_buf, size_t out_buf_size,
                                  bool* classify_output);
...
__NO_RETURN void AlgorithmThread (void *argument) {
  ...
  for (;;) {
    ...
    total_usecase_time = profiler_start();
        ...
        retv = executorch_runner_main(
            sds_algo_data_in_buf, sizeof(sds_algo_data_in_buf), sds_algo_data_out_buf,
            sizeof(sds_algo_data_out_buf), &classify_output);
        if (retv != 0) {
